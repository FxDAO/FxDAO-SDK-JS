...
function generateOptionalVaultKeyScVal(vaultKey) {
    const struct = [];
    struct.push(soroban_client_1.xdr.ScVal.scvSymbol(vaultKey[0]));
    if (vaultKey[0] === 'Some') {
        struct.push(soroban_client_1.xdr.ScVal.scvMap([
            new soroban_client_1.xdr.ScMapEntry({
                key: soroban_client_1.xdr.ScVal.scvSymbol('account'),
                val: (0, soroban_client_1.nativeToScVal)(vaultKey[1].account, { type: 'address' }),
            }),
            new soroban_client_1.xdr.ScMapEntry({
                key: soroban_client_1.xdr.ScVal.scvSymbol('denomination'),
                val: (0, soroban_client_1.nativeToScVal)(vaultKey[1].denomination, { type: 'symbol' }),
            }),
            new soroban_client_1.xdr.ScMapEntry({
                key: soroban_client_1.xdr.ScVal.scvSymbol('index'),
                val: (0, soroban_client_1.nativeToScVal)(vaultKey[1].index, { type: 'u128' }),
            }),
        ]));
    }
    return soroban_client_1.xdr.ScVal.scvVec(struct);
}
...
function errorCodeFromSimulated(response) {
    ...
    try {
        const errorCodeVal = response.events
            ...
            .slice(-1)[0]
            .value();
        errorCode = (0, soroban_client_1.scValToNative)(errorCodeVal);
    }
    catch (e) {
        errorCode = -1;
    }
