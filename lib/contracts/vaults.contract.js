...
class VaultsContract {
    ...
    async calculateDepositRatio(params) {
        const currencyRate = (0, soroban_client_1.nativeToScVal)(params.currencyRate, { type: 'u128' });
        const collateral = (0, soroban_client_1.nativeToScVal)(params.collateral, { type: 'u128' });
        const debt = (0, soroban_client_1.nativeToScVal)(params.debt, { type: 'u128' });
        const account = new soroban_client_1.Account(this.globalParams.simulationAccount, '0');
        ...
        const responseValue = await this.server.simulateTransaction(tx).then(response => {
            ...
            return soroban_client_1.xdr.ScVal.fromXDR(response.result.retval.toXDR());
        });
        return (0, soroban_client_1.scValToBigInt)(responseValue);
    }
    async setPriceRate(params) {
        const account = await this.server.getAccount(params.sourceAccount);
        const rate = (0, soroban_client_1.nativeToScVal)(params.rate, { type: 'u128' });
        const denomination = (0, soroban_client_1.nativeToScVal)(params.denomination, { type: 'symbol' });
        const tx = new soroban_client_1.TransactionBuilder(account, {
            fee: this.globalParams.defaultFee,
            networkPassphrase: this.globalParams.network,
            memo: params.memo,
    ...
    async newVault(params) {
        const prevKey = await this.findPrevVaultKey({
            account: new soroban_client_1.Address(params.caller),
        ...
        const initial_debt = (0, soroban_client_1.nativeToScVal)(params.initialDebt, { type: 'u128' });
        const collateral_amount = (0, soroban_client_1.nativeToScVal)(params.collateralAmount, { type: 'u128' });
        const denomination = (0, soroban_client_1.nativeToScVal)(params.denomination, { type: 'symbol' });
        const tx = new soroban_client_1.TransactionBuilder(account, {
            fee: this.globalParams.defaultFee,
            networkPassphrase: this.globalParams.network,
    ...
    async updateVault(params) {
        ...
        const prev_key = (0, utils_1.generateOptionalVaultKeyScVal)(prevKey);
        const vault_key = soroban_client_1.xdr.ScVal.scvMap([
            new soroban_client_1.xdr.ScMapEntry({
                key: soroban_client_1.xdr.ScVal.scvSymbol('account'),
                val: (0, soroban_client_1.nativeToScVal)(vaultKey.account, { type: 'address' }),
            }),
            new soroban_client_1.xdr.ScMapEntry({
                key: soroban_client_1.xdr.ScVal.scvSymbol('denomination'),
                val: soroban_client_1.xdr.ScVal.scvSymbol(vaultKey.denomination),
            }),
            new soroban_client_1.xdr.ScMapEntry({
                key: soroban_client_1.xdr.ScVal.scvSymbol('index'),
                val: (0, soroban_client_1.nativeToScVal)(vaultKey.index, { type: 'u128' }),
            }),
        ]);
    ...
    async liquidate(params) {
        const account = await this.server.getAccount(params.caller);
        const liquidator = (0, soroban_client_1.nativeToScVal)(account.accountId(), { type: 'address' });
        const denomination = (0, soroban_client_1.nativeToScVal)(params.denomination, { type: 'symbol' });
        const total_vaults_to_liquidate = (0, soroban_client_1.nativeToScVal)(params.totalVaults, { type: 'u32' });
        const tx = new soroban_client_1.TransactionBuilder(account, {
            fee: this.globalParams.defaultFee,
            networkPassphrase: this.globalParams.network,
            memo: params.memo,
    ...
    async getVault(params) {
        const user = (0, soroban_client_1.nativeToScVal)(params.user, { type: 'address' });
        const denomination = (0, soroban_client_1.nativeToScVal)(params.denomination, { type: 'symbol' });
        const tx = new soroban_client_1.TransactionBuilder(new soroban_client_1.Account(this.globalParams.simulationAccount, '0'), {
            fee: this.globalParams.defaultFee,
            networkPassphrase: this.globalParams.network,
        })
            .addOperation(this.contract.call(interfaces_1.FxDAOVaultsContractMethods.set_currency_rate, user, denomination))
            .setTimeout(0)
            .build();
        const simulated = await this.server.simulateTransaction(tx);
        if (isSimulationError(simulated)) {
    ...
    async getVaults(params) {
        ...
        const denomination = (0, soroban_client_1.nativeToScVal)(params.denomination, { type: 'symbol' });
        const total = (0, soroban_client_1.nativeToScVal)(params.total, { type: 'u32' });
        const only_to_liquidate = (0, soroban_client_1.nativeToScVal)(params.onlyToLiquidate, { type: 'bool' });
        const account = new soroban_client_1.Account(this.globalParams.simulationAccount, '0');
        const tx = new soroban_client_1.TransactionBuilder(account, {
            fee: this.globalParams.defaultFee,
            networkPassphrase: this.globalParams.network,
